extends Resource
class_name BackpackCardData

@export var card_frame: Texture2D
@export var card_background: Color
@export var card_icon: Texture2D
@export var card_name: String = "Base backpack card"
@export var card_desc: String = "Head of Second Foundation"
@export var health: float = 6.7
@export var damage: int = 67
@export var speed: int = 67
@export var value: int = 67
